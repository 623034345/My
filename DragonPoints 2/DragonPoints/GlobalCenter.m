...
- (NSString *)dataStr
{
    ...
    NSString *dataStr = [dataFormatter stringFromDate:data];
    NSString *linkStr = [NSString stringWithFormat:@"%@%@", APP_NAME, dataStr];
    return [self md5Str:linkStr];
}
...
{
    if ([@"" isEqualToString:str] || NULL == str || nil == str || [@"(null)" isEqualToString:str] || [@"<null>" isEqualToString:str] || [[NSNull null] isEqual:str])
    {
        return YES;
    }
    return NO;
...
{
    CGSize asize = SCREEN_6P_SIZE;
    
    //如果图片的宽大于6P的宽就按照6P尺寸上传
    if (image.size.width > asize.width)
    {
        ...
        else
        {
            ...
            CGContextRef context = UIGraphicsGetCurrentContext();
            CGContextSetFillColorWithColor(context, UICLEAR.CGColor);
