...
NSComparisonResult compareViewDepth(UIView *view1, UIView *view2, iCarousel *self)
{
    ...
    if (difference == 0.0f)
    {
        ...
        if (self.vertical)
        {
            ...
            difference = fabsf((float)y2 - (float)y3) - fabsf((float)y1 - (float)y3);
        }
        else
        {
            ...
            difference = fabsf((float)x2 - (float)x3) - fabsf((float)x1 - (float)x3);
        }
    }
...
{
    return (fabsf((float)_startVelocity) > SCROLL_SPEED_THRESHOLD) &&
    (fabsf((float)_scrollOffset - (float)self.currentItemIndex) > SCROLL_DISTANCE_THRESHOLD);
}
...
- (void)startDecelerating
{
    ...
    _startTime = CACurrentMediaTime();
    _scrollDuration = fabsf((float)distance) / fabsf(0.5f * (float)_startVelocity);
    ...
    if (_scrollEnabled)
    {
        ...
        switch (panGesture.state)
        {
            ...
            default:
            {
                ...
                if (!_wrapEnabled && _bounces)
                {
                    factor = 1.0f - fminf(fabsf((float)_scrollOffset - (float)[self clampedOffset:_scrollOffset]), _bounceDistance) / _bounceDistance;
                }
